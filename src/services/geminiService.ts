  ...
  async generateNews(category: string = "General"): Promise<NewsArticle[]> {
    ...
    return withRetry(async () => {
      const response = await ai.models.generateContent({
        ...
        Return them as a JSON array of objects with: id, title, content (around 100 words), category, author, date (set this to "May 9, 2026"), and a descriptive imageUrl placeholder (e.g., https://picsum.photos/seed/news1/800/600).`,
        config: {
          tools: [{ googleSearch: {} }],
          responseMimeType: "application/json",
          responseSchema: {
